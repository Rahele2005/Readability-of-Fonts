...
- Solution: The number of participants were increased to more than 10 to ensure that we have at least 9-10 participants with proper calibration and the experiment was run a few times before the participant starts.
...
The functions are employed to detect the gaze before the sentence appears preventing any odds of outliers in fixations. Below are the images of how it looks like on Open Sesame (fig. 4,5,6):
...
### Fixation Duration: 
Fixation duration is the length of time a participant's gaze remains fixed on a single location. 
...
The data gathered for number and duration of fixations and landing spot showed no big difference between the two case formats. Additionally, for reasons explained previousely not all participants' data could be analysed. Figures 16 and 17 reveal overall results for two of our participants in our experiment. The logs, however, are nor visible for all of them due to the defected data from the eye tracker. 
